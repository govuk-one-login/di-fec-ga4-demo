...
describe("New Tab Link", () => {
  describe("by default", () => {
    ...
    it("renders an English element as text and Welsh as a link", () => {
      ...
      expect($component.data("journeyClick")).toEqual(
        "link - click:lang-select:Cymraeg"
      );

      expect($component.find(".govuk-visually-hidden").eq(0).text()).toEqual(
        "Newid yr iaith ir Gymraeg"
      );
      expect($component.find('[aria-hidden="true"]').eq(0).text()).toEqual(
        "Cymraeg"
      );
      expect(
        $.text()
          .trim()
          .replace(/[\s,\n]+/g, " ")
      ).toEqual("English Newid yr iaith ir Gymraeg Cymraeg");

      expect(
        $(".onelogin-language-select__list-item")
    ...
    it("renders a Welsh element as text and English as a link", () => {
      ...
      expect(
        $.text()
          .trim()
          .replace(/[\s,\n]+/g, " ")
      ).toEqual("Change the language to English English Cymraeg");
