...
Given('I have neither accepted nor rejected analytics cookies', async function() {
...
Then('The dataLayer includes the welcome page view event', async function() {
  ...
  const pageViewEvent = dataLayer.find(eventItem => eventItem.event === 'page_view_ga4').page_view;
...
Then('The dataLayer includes the organisation type page view event', async function() {
  const dataLayer = await this.page.evaluate('window.dataLayer');
  const url = new URL(this.page.url());
  const pageViewEvent = dataLayer.find(eventItem => eventItem.event === 'page_view_ga4').page_view;
  expect(pageViewEvent).to.contain({
...
Then('The dataLayer includes the help with hint page view event', async function() {
  const dataLayer = await this.page.evaluate('window.dataLayer');
  const url = new URL(this.page.url());
  const pageViewEvent = dataLayer.find(eventItem => eventItem.event === 'page_view_ga4').page_view;
  expect(pageViewEvent).to.contain({
...
Then('The dataLayer includes the service description page view event', async function() {
  const dataLayer = await this.page.evaluate('window.dataLayer');
  const url = new URL(this.page.url());
  const pageViewEvent = dataLayer.find(eventItem => eventItem.event === 'page_view_ga4').page_view;
  expect(pageViewEvent).to.contain({
...
Then('The dataLayer includes the choose location page view event', async function() {
  const dataLayer = await this.page.evaluate('window.dataLayer');
  const url = new URL(this.page.url());
  const pageViewEvent = dataLayer.find(eventItem => eventItem.event === 'page_view_ga4').page_view;
  expect(pageViewEvent).to.contain({
...
Then('The dataLayer includes the choose enter email page view event', async function() {
  const dataLayer = await this.page.evaluate('window.dataLayer');
  const url = new URL(this.page.url());
  const pageViewEvent = dataLayer.find(eventItem => eventItem.event === 'page_view_ga4').page_view;
  expect(pageViewEvent).to.contain({
...
Then('The dataLayer includes the navigation button event', async function () {
  ...
  const eventData = dataLayer.find(eventItem => eventItem.event === 'event_data').event_data;
...
Then('The dataLayer includes the navigation inbound link event', async function () {
  const dataLayer = this.context.dataLayer;
  const eventData = dataLayer.find(eventItem => eventItem.event === 'event_data').event_data;
  expect(eventData).to.contain({
      'event_name': 'navigation',
...
Then('The dataLayer includes the navigation outbound link event', async function () {
  const dataLayer = this.context.dataLayer;
  const eventData = dataLayer.find(eventItem => eventItem.event === 'event_data').event_data;
  expect(eventData).to.contain({
      'event_name': 'navigation',
...
Then('The dataLayer includes the navigation back link event', async function () {
  const dataLayer = this.context.dataLayer;
  const eventData = dataLayer.find(eventItem => eventItem.event === 'event_data').event_data;
  expect(eventData).to.contain({
      'event_name': 'navigation',
...
Then('The dataLayer includes the navigation banner link event', async function () {
  const dataLayer = this.context.dataLayer;
  const eventData = dataLayer.find(eventItem => eventItem.event === 'event_data').event_data;
  expect(eventData).to.contain({
      'event_name': 'navigation',
...
Then('The dataLayer includes the navigation footer link event', async function () {
  const dataLayer = this.context.dataLayer;
  const eventData = dataLayer.find(eventItem => eventItem.event === 'event_data').event_data;
  expect(eventData).to.contain({
      'event_name': 'navigation',
...
Then('The dataLayer includes the navigation footer licence link event', async function () {
  const dataLayer = this.context.dataLayer;
  const eventData = dataLayer.find(eventItem => eventItem.event === 'event_data').event_data;
  expect(eventData).to.contain({
      'event_name': 'navigation',
...
Then('The dataLayer includes the navigation footer copyright link event', async function () {
  const dataLayer = this.context.dataLayer;
  const eventData = dataLayer.find(eventItem => eventItem.event === 'event_data').event_data;
  expect(eventData).to.contain({
      'event_name': 'navigation',
      ...
      'section': 'copyright',
      'action': 'undefined',
      'url': `https://www.nationalarchives.gov.uk/information-management/re-using-public-sector-information/uk-gov`,
      'external': 'true',
      'link_domain': `https://www.nationalarchives.gov.uk`,
      'link_path_parts.1': '/information-management/re-using-public-sector-information/uk-government-licensing-framework/crown-copyright/',
      'link_path_parts.2': 'undefined',
      'link_path_parts.3': 'undefined',
      'link_path_parts.4': 'undefined',
      'link_path_parts.5': 'undefined'
...
Then('The dataLayer includes the navigation menu link event', async function () {
  const dataLayer = this.context.dataLayer;
  const eventData = dataLayer.find(eventItem => eventItem.event === 'event_data').event_data;
  expect(eventData).to.contain({
      'event_name': 'navigation',
...
Then('The dataLayer includes the very long navigation menu link event', async function () {
  const dataLayer = this.context.dataLayer;
  const eventData = dataLayer.find(eventItem => eventItem.event === 'event_data').event_data;
  expect(eventData).to.contain({
      'event_name': 'navigation',
      ...
      'section': 'undefined',
      'action': 'undefined',
      'url': `http://localhost:3000/xdncnmqhevoqoeotnvmwunsubywwkwwapuzawrybnzgrdocobszfqmpwaxqrphmuehztfzgjjufjot`,
      'external': 'false',
      'link_domain': `http://localhost:3000`,
      'link_path_parts.1': '/xDncNmqheVoQoeOTnVmwUnsuByWwKwwAPUZAWRYBnzgrDOCObSzFqMpwAxQRpHMUehzTfzGJjuFJOtWyQBdHQbtpEpxmopVEnghdxyz',
      'link_path_parts.2': 'undefined',
      'link_path_parts.3': 'undefined',
      'link_path_parts.4': 'undefined',
      'link_path_parts.5': 'undefined'
...
Then('The dataLayer includes the form error event into organisation type error page', async function () {
  const dataLayer = await this.page.evaluate('window.dataLayer');
  const eventData = dataLayer.find(eventItem => eventItem.event === 'event_data').event_data;
  expect(eventData).to.contain({
...
Then('The dataLayer includes the form error event into help with hint error page', async function () {
  const dataLayer = await this.page.evaluate('window.dataLayer');
  const eventData = dataLayer.find(eventItem => eventItem.event === 'event_data').event_data;
  expect(eventData).to.contain({
      "event_name": "form_error",
...
Then('The dataLayer includes the form error event into service description error page', async function () {
  const dataLayer = await this.page.evaluate('window.dataLayer');
  const eventData = dataLayer.find(eventItem => eventItem.event === 'event_data').event_data;
  expect(eventData).to.contain({
      "event_name": "form_error",
...
Then('The dataLayer includes the form error event into choose location error page', async function () {
  const dataLayer = await this.page.evaluate('window.dataLayer');
  const eventData = dataLayer.find(eventItem => eventItem.event === 'event_data').event_data;
  expect(eventData).to.contain({
      "event_name": "form_error",
...
Then('The dataLayer includes the form error event into enter email error page', async function () {
  const dataLayer = await this.page.evaluate('window.dataLayer');
  const eventData = dataLayer.find(eventItem => eventItem.event === 'event_data').event_data;
  expect(eventData).to.contain({
      "event_name": "form_error",
...
Then('The dataLayer includes the organisation type form response event', async function () {
  const dataLayer = this.context.dataLayer;
  const eventData = dataLayer.find(eventItem => eventItem.event === 'event_data').event_data;
  expect(eventData).to.contain({
...
Then('The dataLayer includes the help with hint form response event', async function () {
  const dataLayer = this.context.dataLayer;
  const eventData = dataLayer.find(eventItem => eventItem.event === 'event_data').event_data;
  expect(eventData).to.contain({
      'event_name': 'form_response',
...
Then('The dataLayer includes the service description form response event', async function () {
  const dataLayer = this.context.dataLayer;
  const eventData = dataLayer.find(eventItem => eventItem.event === 'event_data').event_data;
  expect(eventData).to.contain({
      'event_name': 'form_response',
...
Then('The dataLayer includes the enter email form response event', async function () {
  const dataLayer = this.context.dataLayer;
  const eventData = dataLayer.find(eventItem => eventItem.event === 'event_data').event_data;
  expect(eventData).to.contain({
      'event_name': 'form_response',
...
Then('The dataLayer includes the choose location form response event', async function () {
  const dataLayer = this.context.dataLayer;
  const eventData = dataLayer.find(eventItem => eventItem.event === 'event_data').event_data;
  expect(eventData).to.contain({
      'event_name': 'form_response',
...
Given('I set up a listener for the data layer push', async function () {
  this.page.on('console', (message) => {
    ...
    const rebuildOriginalEvent = (stringifiedEvent) => {
      ...
      const undefinedValuesObject = createObjectWithKeys(deconstructedEvent.undefinedValues ?? [])
