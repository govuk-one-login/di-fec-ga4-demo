...
Given("I visit the {word} page", { timeout: 30 * 1000 }, async function (title) {
...
When("I choose {word} value", async function (text) {
  ...
  await this.page.evaluate(`document.querySelector('option[value="${text}"]').selected = true`);
  await this.page.waitForLoadState('networkidle'); // Wait for network calls to finish
})

When("I click Accept", async function () {
  await this.page.locator(`button:has-text("Accept") >> visible = true`).click();
  await this.page.waitForLoadState('networkidle'); // Wait for network calls to finish
});

When("I click Reject", async function () {
  await this.page.locator(`button:has-text("Reject") >> visible = true`).click();
  await this.page.waitForLoadState('networkidle'); // Wait for network calls to finish
});
...
When("I click {word} {word}", async function (text, element) {
  ...
  await this.page.locator(`${element}:text-is("${text}") >> visible = true`).click();
  await this.page.waitForLoadState('networkidle'); // Wait for network calls to finish
});

When("I click {word} {word} {word}", async function (text, text2, element) {
    if (element === "link") element = "a";
    await this.page.locator(`${element}:has-text("${text} ${text2}") >> visible = true`).click();
    await this.page.waitForLoadState('networkidle'); // Wait for network calls to finish
});


When("I click {word} {word} {word} {word}", async function (text, text2, text3, element) {
  if (element === "link") element = "a";
  await this.page.locator(`${element}:has-text("${text} ${text2} ${text3}") >> visible = true`).click();
  await this.page.waitForLoadState('networkidle'); // Wait for network calls to finish
});
...
Then("The {word} cookie is set to {word}", async function (cookieName, cookieValue) {
